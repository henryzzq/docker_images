...
RUN wget --no-check-certificate 'https://docs.google.com/uc?export=download&id=0B-NEimEr29Wdd00ySlhjdm8tSVU' -O create-osb-cluster.py
...
RUN chmod -R a+x /startWL_DB.sh /create-osb-cluster.py /root/Oracle/Middleware/Oracle_OSB1/common/bin

RUN mkdir -p /util && \
    mv /startWL_DB.sh /util

RUN /root/Oracle/Middleware/Oracle_OSB1/common/bin/wlst.sh -skipWLSModuleScanning /create-osb-cluster.py
